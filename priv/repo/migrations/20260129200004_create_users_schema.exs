defmodule Animetana.Repo.Migrations.CreateUsersSchema do
  @moduledoc """
  Creates the users schema for Animetana.

  This schema contains ALL user-related data:
  - User accounts, authentication, settings
  - Anime/Manga lists and progress tracking
  - Favorites, follows, reviews
  - Discussion threads (anime/manga/episode discussions)
  - Notifications, activities
  - Reports and moderation

  Note: Chapter-level progress tracking is NOT included.
  Manga progress is tracked at the manga level (chapters read count).
  """
  ...
  defp create_enum_types do
    ...
    # Notification type (removed new_chapter)
    execute """
    ...
    # Thread type (renamed from chat_room_type, removed chapter)
    execute """
    CREATE TYPE users.thread_type AS ENUM (
        'anime', 'manga', 'episode', 'general', 'private'
    )
    """, "DROP TYPE IF EXISTS users.thread_type"

    # Thread participant role (renamed from chat_participant_role)
    execute """
    CREATE TYPE users.thread_participant_role AS ENUM (
        'owner', 'admin', 'moderator', 'member'
    )
    """, "DROP TYPE IF EXISTS users.thread_participant_role"
  ...
  # ============================================================================
  # USER SETTINGS TABLE (removed chapter notification settings)
  # ============================================================================
  ...
  # ============================================================================
  # THREAD TABLES (renamed from chat)
  # ============================================================================
  defp create_thread_tables do
    # Discussion threads
    execute """
    CREATE TABLE users.threads (
        id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),

        -- Thread info
        type users.thread_type NOT NULL DEFAULT 'general',
        title VARCHAR(500) NOT NULL,
        description TEXT,

        -- Content association (no chapter_id since we removed chapter tracking)
        ...
        -- Constraints (removed chapter type)
        CONSTRAINT threads_content_check CHECK (
            ...
            (type IN ('general', 'private') AND anime_id IS NULL AND manga_id IS NULL AND episode_id IS NULL)
        )
    )
    """, "DROP TABLE IF EXISTS users.threads CASCADE"

    # Thread messages
    execute """
    CREATE TABLE users.thread_messages (
        id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
        thread_id UUID NOT NULL REFERENCES users.threads(id) ON DELETE CASCADE,
        ...
        -- Reply threading
        reply_to_id UUID REFERENCES users.thread_messages(id) ON DELETE SET NULL,
    ...
    """, "DROP TABLE IF EXISTS users.thread_messages CASCADE"

    # Thread participants
    execute """
    CREATE TABLE users.thread_participants (
        thread_id UUID NOT NULL REFERENCES users.threads(id) ON DELETE CASCADE,
        user_id BIGINT NOT NULL REFERENCES users.users(id) ON DELETE CASCADE,

        -- Role
        role users.thread_participant_role DEFAULT 'member',
    ...
    """, "DROP TABLE IF EXISTS users.thread_participants CASCADE"
  end
  ...
  defp create_all_indexes do
    ...
    # Thread indexes (renamed from chat)
    execute "CREATE INDEX idx_threads_type_content ON users.threads (type, anime_id, manga_id)"
    execute "CREATE INDEX idx_threads_active ON users.threads (is_active, last_message_at DESC)"

    execute "CREATE INDEX idx_thread_messages_thread_time ON users.thread_messages (thread_id, inserted_at DESC)"
    execute "CREATE INDEX idx_thread_messages_user ON users.thread_messages (user_id, inserted_at DESC)"
    execute "CREATE INDEX idx_thread_messages_reply ON users.thread_messages (reply_to_id) WHERE reply_to_id IS NOT NULL"

    execute "CREATE INDEX idx_thread_participants_user ON users.thread_participants (user_id, last_seen_at DESC)"
    execute "CREATE INDEX idx_thread_participants_thread ON users.thread_participants (thread_id) WHERE is_banned = FALSE"
  ...
  defp create_functions do
    ...
    # Update thread stats (renamed from chat)
    execute """
    CREATE OR REPLACE FUNCTION users.update_thread_stats() RETURNS TRIGGER AS $$
    BEGIN
        IF TG_TABLE_NAME = 'thread_messages' THEN
            UPDATE users.threads SET
                message_count = (
                    SELECT COUNT(*) FROM users.thread_messages
        ...
        IF TG_TABLE_NAME = 'thread_participants' THEN
            UPDATE users.threads SET
                participant_count = (
                    SELECT COUNT(*) FROM users.thread_participants
                    WHERE thread_id = COALESCE(NEW.thread_id, OLD.thread_id)
                    AND is_banned = FALSE
                ),
                updated_at = NOW()
    ...
    END;
    $$ LANGUAGE plpgsql
    """, "DROP FUNCTION IF EXISTS users.update_thread_stats()"
  ...
  defp create_triggers do
    ...
    # Thread stats triggers (renamed from chat)
    execute """
    CREATE TRIGGER thread_messages_stats_trigger
        AFTER INSERT OR UPDATE OR DELETE ON users.thread_messages
        FOR EACH ROW EXECUTE FUNCTION users.update_thread_stats()
    """

    execute """
    CREATE TRIGGER thread_participants_stats_trigger
        AFTER INSERT OR UPDATE OR DELETE ON users.thread_participants
        FOR EACH ROW EXECUTE FUNCTION users.update_thread_stats()
    """
    ...
    CREATE TRIGGER threads_updated_at_trigger
        BEFORE UPDATE ON users.threads
        FOR EACH ROW EXECUTE FUNCTION users.update_updated_at()
    """

    execute """
    CREATE TRIGGER thread_messages_updated_at_trigger
        BEFORE UPDATE ON users.thread_messages
        FOR EACH ROW EXECUTE FUNCTION users.update_updated_at()
    """
  ...
  defp create_views do
    ...
    SELECT
        ...
        a.title_en,
        a.title_ja,
        a.cover_image_large AS cover_image_url,
        a.format AS type,
        a.status AS anime_status,
        a.episodes AS total_episodes,
        a.Animetana_score_en AS Animetana_score
  ...
  defp create_comments do
    ...
    execute "COMMENT ON TABLE users.user_manga_lists IS 'User manga tracking lists with progress and ratings (chapters tracked at manga level)'"
    ...
    execute "COMMENT ON TABLE users.threads IS 'Discussion threads for anime, manga, episodes, and general topics'"
    execute "COMMENT ON TABLE users.thread_messages IS 'Messages in discussion threads with threading support'"
